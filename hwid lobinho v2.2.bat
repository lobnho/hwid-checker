...
powershell -NoLogo -NoProfile -ExecutionPolicy Bypass -Command "Get-CimInstance Win32_NetworkAdapter | Where-Object { $_.MACAddress } | ForEach-Object { Write-Host 'Adapter: ' -NoNewline; Write-Host $_.Description -NoNewline -ForegroundColor White; Write-Host ' | MAC: ' -NoNewline -ForegroundColor DarkGray; Write-Host $_.MACAddress -ForegroundColor Magenta }"
echo.
...
echo %C_RESET%
powershell -NoLogo -NoProfile -ExecutionPolicy Bypass -Command "function Short([string]$s,[int]$n){ if([string]::IsNullOrWhiteSpace($s)){return 'N/A'}; if($s.Length -gt $n){return $s.Substring(0,$n-3)+'...'}; return $s }; $roots=@('HKLM:\SYSTEM\CurrentControlSet\Enum\USB','HKLM:\SYSTEM\CurrentControlSet\Enum\USBSTOR'); $items=foreach($root in $roots){ if(Test-Path $root){ Get-ChildItem $root -ErrorAction SilentlyContinue | ForEach-Object { $deviceKey=$_.PSChildName; Get-ChildItem $_.PSPath -ErrorAction SilentlyContinue | ForEach-Object { $p=Get-ItemProperty $_.PSPath -ErrorAction SilentlyContinue; $desc=if($p.DeviceDesc){($p.DeviceDesc -split ';')[-1]}else{$deviceKey}; $name=if($p.FriendlyName){$p.FriendlyName}elseif($p.LocationInformation){$p.LocationInformation}else{$desc}; $class=if($p.Class){$p.Class}else{'Unknown'}; $probe=($desc + ' ' + $name + ' ' + $class + ' ' + $root); $type=if($probe -match 'HID|USB Input|Keyboard|Mouse'){'HID (Human Interface Device)'}elseif($probe -match 'Mass Storage|Disk drive|USBSTOR'){'Mass Storage'}elseif($probe -match 'Audio'){'Audio'}elseif($probe -match 'WinUsb|Vendor'){'Vendor Specific'}elseif($probe -match 'Hub|Composite'){'Unknown'}elseif($class -ne 'Unknown'){$class}else{'Unknown'}; [pscustomobject]@{Name=$name;Description=$desc;Type=$type;Serial=$_.PSChildName} } } } }; $items=@($items | Sort-Object Name,Description,Serial -Unique); if($items.Count -eq 0){ Write-Host 'No USB devices found.' -ForegroundColor Red } else { Write-Host 'Device Name | Description | Device Type | Serial Number' -ForegroundColor White; $items | ForEach-Object { Write-Host (Short $_.Name 26) -NoNewline -ForegroundColor Magenta; Write-Host ' | ' -NoNewline -ForegroundColor DarkGray; Write-Host (Short $_.Description 28) -NoNewline -ForegroundColor Yellow; Write-Host ' | ' -NoNewline -ForegroundColor DarkGray; Write-Host (Short $_.Type 28) -NoNewline -ForegroundColor Yellow; Write-Host ' | ' -NoNewline -ForegroundColor DarkGray; Write-Host $_.Serial -ForegroundColor Magenta } }"
