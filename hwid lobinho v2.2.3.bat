...
powershell -Command "$h = Get-Host; $w = $h.UI.RawUI; $b = $w.BufferSize; $b.Width = 240; $b.Height = 9999; $w.BufferSize = $b;" >nul 2>&1
...
echo CTRL + F2 TO COMPARE SERIALS
echo %C_RESET%

:waitAction
set "ACTION="
for /f "delims=" %%k in ('powershell -NoLogo -NoProfile -ExecutionPolicy Bypass -Command "[Console]::TreatControlCAsInput=$true; $k=$Host.UI.RawUI.ReadKey('NoEcho,IncludeKeyDown'); $ctrl=($k.ControlKeyState.ToString() -match 'LeftCtrlPressed|RightCtrlPressed'); if($k.VirtualKeyCode -eq 113 -and $ctrl){'COMPARE'} elseif($k.VirtualKeyCode -eq 116 -and $ctrl){'SAVE'} elseif($k.VirtualKeyCode -eq 116){'REFRESH'} elseif(($k.Character -eq [char]3) -or ($k.VirtualKeyCode -eq 67 -and $ctrl)){'COPY'} else {'WAIT'}"') do set "ACTION=%%k"
...
:CompareSerials
powershell -NoLogo -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Stop'; function Get-ConsoleText{ try{ $raw=$Host.UI.RawUI; $width=$raw.BufferSize.Width; $bottom=$raw.CursorPosition.Y; $rect=New-Object System.Management.Automation.Host.Rectangle 0,0,($width-1),$bottom; $cells=$raw.GetBufferContents($rect); $lines=for($y=0;$y -le $bottom;$y++){ $chars=for($x=0;$x -lt $width;$x++){ $cells[$y,$x].Character }; (-join $chars).TrimEnd() }; return (($lines -join [Environment]::NewLine).TrimEnd()) }catch{return ''} }; function Section-Name([string]$s){ $x=$s.Trim('[',']',' '); if($x -match 'SYSTEM'){return 'SYSTEM'}; if($x -match 'WINDOWS'){return 'WINDOWS'}; if($x -match 'ADVANCED'){return 'ADVANCED'}; if($x -match 'MONITOR'){return 'MONITOR'}; if($x -match 'FUSER'){return 'FUSER'}; if($x -match 'DISK DRIVES'){return 'DISK'}; if($x -match 'S\.M\.A\.R\.T'){return 'SMART'}; if($x -match 'VOLUME'){return 'VOLUME'}; if($x -match 'NETWORK'){return 'NETWORK'}; if($x -match 'USB DEVICES'){return 'USB'}; if($x -match 'BLUETOOTH'){return 'BLUETOOTH'}; if($x -match 'MEDIA'){return 'MEDIA'}; if($x -match 'PCI'){return 'PCI'}; if($x -match 'CPU'){return 'CPU'}; if($x -match 'RAM'){return 'RAM'}; if($x -match 'GPU'){return 'GPU'}; if($x -match 'TPM|SECURITY'){return 'TPM'}; return $x }; function Ignore-Line([string]$line){ return ($line -match '^\s*$' -or $line -match '^(F5|CTRL|Serials copied|Serial file saved|Select|Type|SAVED SERIAL FILES|COMPARE WITH|COMPARE RESULT|Unchanged|No serial|Invalid|No saved|[0-9]+\. )') }; function Parts([string]$line){ @($line -split '\s+\|\s+' | ForEach-Object { $_.Trim() } | Where-Object { $_ }) }; function Has-Key($map,[string]$key){ return $map.Contains($key) }; function Clean([string]$s){ if($null -eq $s){return ''}; return ($s.Trim().TrimStart('|','>',' ')).Trim() }; function Add($map,[string]$section,[string]$object,[string]$key,[string]$value){ $key=Clean $key; $value=Clean $value; $object=Clean $object; if(-not $key -or -not $value){return}; $label=if($object){$section + ' ' + $object + ' ' + $key}else{$section + ' ' + $key}; if(-not (Has-Key $map $label)){ $map[$label]=$value; return }; $i=2; while(Has-Key $map ($label + ' #' + $i)){ $i++ }; $map[$label + ' #' + $i]=$value }; function Pair([string]$part){ if($part -match '^(?<k>[^:]+):\s*(?<v>.*)$'){ return @((Clean $Matches.k),(Clean $Matches.v)) }; return $null }; function Parse-Serials([string]$text){ $map=[ordered]@{}; $section='GENERAL'; foreach($raw in ($text -split '\r?\n')){ try{ $line=$raw.Trim(); if($line -match '^\s*\[(.+?)\]\s*$'){ $section=Section-Name $line; continue }; if(Ignore-Line $line){continue}; $parts=Parts $line; if($parts.Count -eq 0){continue}; $pairs=@(); $free=@(); foreach($p in $parts){ $pair=Pair $p; if($pair){ $pairs += ,$pair } elseif($p -match '^(?<name>.+?)\s+Serial:\s*(?<serial>.+)$'){ $free += Clean $Matches.name; $pairs += ,@('Serial',(Clean $Matches.serial)) } else { $free += Clean $p } }; if($pairs.Count -eq 0){continue}; $obj=''; foreach($candidate in @('Model','Disk','Volume','Adapter','Name','Class')){ $hit=$pairs | Where-Object { $_[0] -eq $candidate } | Select-Object -First 1; if($hit){ $obj=$hit[1]; break } }; if(-not $obj -and $free.Count -gt 0){ $obj=$free[0] }; foreach($pair in $pairs){ $k=$pair[0]; $v=$pair[1]; if($section -in @('SYSTEM','WINDOWS','ADVANCED','MONITOR','FUSER','NETWORK','TPM','CPU','GPU') -and $k -notin @('Name','Class')){ Add $map $section '' $k $v } elseif($k -in @('Model','Disk','Volume','Adapter','Name') -and $section -notin @('SYSTEM','WINDOWS','MONITOR','CPU','GPU')){ Add $map $section $v 'Present' 'YES' } else { Add $map $section $obj $k $v } } }catch{} }; return $map }; try{ $desktop=[Environment]::GetFolderPath('DesktopDirectory'); $folder=Join-Path $desktop 'Serials from HWID CHECKER'; if(-not (Test-Path $folder)){ Write-Host 'No saved serial folder found. Save a serial file first with CTRL + F5.' -ForegroundColor Yellow; return }; $files=@(Get-ChildItem -LiteralPath $folder -Filter '*.txt' -File | Sort-Object LastWriteTime -Descending); if($files.Count -lt 1){ Write-Host 'No saved serial files found. Save a serial file first with CTRL + F5.' -ForegroundColor Yellow; return }; Write-Host ''; Write-Host 'SAVED SERIAL FILES' -ForegroundColor Cyan; for($i=0;$i -lt $files.Count;$i++){ Write-Host (($i+1).ToString() + '. ') -NoNewline -ForegroundColor Yellow; Write-Host ($files[$i].LastWriteTime.ToString('yyyy-MM-dd HH:mm:ss') + ' | ' + $files[$i].Name) -ForegroundColor Magenta }; $baseInput=Read-Host 'Select BASE serial file number'; $parsed=0; if(-not [int]::TryParse($baseInput,[ref]$parsed)){ Write-Host 'Invalid selection.' -ForegroundColor Red; return }; $baseIndex=$parsed; if($baseIndex -lt 1 -or $baseIndex -gt $files.Count){ Write-Host 'Invalid selection.' -ForegroundColor Red; return }; Write-Host ''; Write-Host 'COMPARE WITH' -ForegroundColor Cyan; Write-Host '0. CURRENT SERIALS' -ForegroundColor Green; for($i=0;$i -lt $files.Count;$i++){ Write-Host (($i+1).ToString() + '. ') -NoNewline -ForegroundColor Yellow; Write-Host ($files[$i].LastWriteTime.ToString('yyyy-MM-dd HH:mm:ss') + ' | ' + $files[$i].Name) -ForegroundColor Magenta }; $newInput=Read-Host 'Select NEW serial file number, or 0 for current'; $parsed=0; if(-not [int]::TryParse($newInput,[ref]$parsed)){ Write-Host 'Invalid selection.' -ForegroundColor Red; return }; $newIndex=$parsed; if($newIndex -lt 0 -or $newIndex -gt $files.Count){ Write-Host 'Invalid selection.' -ForegroundColor Red; return }; $baseText=Get-Content -LiteralPath $files[$baseIndex-1].FullName -Raw; $newText=if($newIndex -eq 0){ Get-ConsoleText }else{ Get-Content -LiteralPath $files[$newIndex-1].FullName -Raw }; $old=Parse-Serials $baseText; $new=Parse-Serials $newText; $keyList=New-Object System.Collections.Generic.List[string]; foreach($k in $old.Keys){ if(-not $keyList.Contains([string]$k)){ $keyList.Add([string]$k) } }; foreach($k in $new.Keys){ if(-not $keyList.Contains([string]$k)){ $keyList.Add([string]$k) } }; $changed=0; $same=0; Write-Host ''; Write-Host 'COMPARE RESULT' -ForegroundColor Cyan; foreach($key in $keyList){ $a=if(Has-Key $old $key){$old[$key]}else{'<MISSING>'}; $b=if(Has-Key $new $key){$new[$key]}else{'<MISSING>'}; if($a -eq $b){$same++; continue}; $changed++; Write-Host ('#' + $changed + ' | ' + $key + ': ') -NoNewline -ForegroundColor Yellow; Write-Host $a -NoNewline -ForegroundColor DarkGray; Write-Host ' >> ' -NoNewline -ForegroundColor White; Write-Host $b -ForegroundColor Green }; if($changed -eq 0){ Write-Host 'No serial changes found.' -ForegroundColor Green }; Write-Host ('Unchanged entries: ' + $same) -ForegroundColor Yellow }catch{ Write-Host ('Compare failed: ' + $_.Exception.Message) -ForegroundColor Red }"
exit /b
